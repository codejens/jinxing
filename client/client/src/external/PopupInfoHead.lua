...
function PopupInfoHead:createNormalCircle( useritem ,headSize,circleframe)
	local sf = PopupInfoHead.new()
	local head = HeadSprite:createNormalCircle(useritem, headSize,circleframe)
	sf.m_useritem = useritem
	sf.m_head = head
	if nil ~= head then
		sf:addChild(head)
