...
function Notifier:sendNotification(notificationName, body, datatype)
	assert(type(notificationName) == "string", "notificationName expected is string")
	local facade = self:getFacade()
	if facade ~= nil then
		facade:sendNotification(notificationName, body, datatype)
	end
end
--[[
