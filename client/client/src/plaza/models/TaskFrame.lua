...
function TaskFrame:ctor(view,callbcak,kind)
	TaskFrame.super.ctor(self,view,callbcak)

	self.m_tabTaskInfo = {}
	self.m_tabTaskInfo.tasklist = {}
	
	self.m_filterGameKind = kind or 0
...
function TaskFrame:onSubTaskList(pData)
	--计算任务数目
	...
	for i = 1,itemcount do
		...
		if self.m_filterGameKind ~= 0 then
			if item.wKindID == self.m_filterGameKind then
