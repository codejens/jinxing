...
function ClientScene:initGameList()
    ...
    local shareurl = GlobalUserItem.getShareUrl()
	self._qrcode = QrNode:createQrNode(shareurl, 125, 5, 1)
	-- self._qrcode:setPosition(137,278)
	local bt_ads = pageViewTouchEventMask:getChildByName("bt_ads")
    bt_ads:getChildByName("img_erwei"):addChild(self._qrcode)
...
function ClientScene:onGameClickEvent(sender)
    ...
    if kinid==410 then
        if GlobalUserItem.lUserScore > 10 then
        local action="gotolibocai"
     local url = string.format("%s/WS/Account.ashx?action=%s&userid=%d&ip=%s", yl.GAME_SHISHICAI,action,GlobalUserItem.dwUserID,string.urlencode(GlobalUserItem.szIpAdress) )
			--
            MultiPlatform:getInstance():openWebview(url)
    else
    local runScene = cc.Director:getInstance():getRunningScene()
          --  showToast(, url, 1)
        showToast(runScene, "真遗憾!玩家金币大于 " .. 10 .. " 才可以进入游戏!", 1)
        return
        end
    end 
	--检测游戏更新
...
--获取互动动画
--function ClientScene:getAniByFile(fileName, armaName, pos, parent)
--	ccs.ArmatureDataManager:getInstance():addArmatureFileInfo(fileName)
--	local armature = ccs.Armature:create(armaName) 
--	armature:setPosition(pos)
--    if parent then
--	    parent:addChild(armature, 1)
--    end
