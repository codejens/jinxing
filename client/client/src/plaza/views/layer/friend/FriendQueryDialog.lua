...
function FriendQueryDialog:ctor(msgTab, callback, txtsize, queryType)
    ...
    cc.Label:createWithTTF("提示", "base/fonts/round_body.ttf", 36)
    ...
    local mainText = cc.Label:createWithTTF(mainMsg, "base/fonts/round_body.ttf", not txtsize and FriendQueryDialog.DEF_TEXT_SIZE or txtsize)
    ...
    if "" ~= subMsg then
        ...
        cc.Label:createWithTTF(subMsg, "base/fonts/round_body.ttf", not txtsize and FriendQueryDialog.DEF_TEXT_SIZE or txtsize)
            :setTextColor(cc.c4b(255,255,255,255))
            :setAnchorPoint(cc.p(0.5,0.5))
            :setDimensions(600, 0)
            :setHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER)
