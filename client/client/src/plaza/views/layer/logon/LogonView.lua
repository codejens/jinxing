...
function LogonView:ctor(serverConfig, viewBase)
	...
	--账号登录
	btnAccountsLogon = self._csbNode:getChildByName("btnAccountLogon")
    ...
    --找回密码
	btnRetPassword = self._csbNode:getChildByName("btnPasswd")
    btnRetPassword:setPressedActionEnabled(true)
		:setTag(LogonView.BT_FGPW)
		:setName("btn_4")
	btnRetPassword:addTouchEventListener(btcallback)
 


	--游客登录
	print("游客登录游客登录游客登录游客登录游客登录游客登录游客登录游客登录游客登录游客登录游客登录游客登录")
...
function LogonView:onButtonPEnabled()
  --btnRetPassword:setEnabled(false)
end

function LogonView:onButtonClickedEvent(tag,ref)
	ExternalFun.playClickEffect()
	print("ttttttttttttttttttt=",tag)
