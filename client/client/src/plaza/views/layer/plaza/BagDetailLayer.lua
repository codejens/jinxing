...
function BagDetailLayer:ctor(scene, gameFrame)
	...
	local area = ""
	if bit.band(self._item._area,yl.PT_ISSUE_AREA_PLATFORM) then
		area = "大厅适用 "
	end
	if bit.band(self._item._area,yl.PT_ISSUE_AREA_SERVER) then
		area = area.."房间适用 "
	end
	if bit.band(self._item._area,yl.PT_ISSUE_AREA_GAME) then
		area = area.."游戏适用 "
	end
	cc.Label:createWithTTF(area, "base/fonts/round_body.ttf", 22)
        	:setAnchorPoint(cc.p(0.0,0.5))
        	:move(620,360)
	...
	--功能描述
	cc.Label:createWithTTF("功能："..self._item._info, "base/fonts/round_body.ttf", 22)
        	:setAnchorPoint(cc.p(0.5,0.5))
        	:move(yl.WIDTH/2,70)
