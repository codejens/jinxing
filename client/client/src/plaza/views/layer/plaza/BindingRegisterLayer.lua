...
--获取父场景节点(ClientScene)
function BindingRegisterLayer:getRootNode()
...
function BindingRegisterLayer:onModifyCallBack(result,message)
    ...
    if result == 2 then
        ...
        self:runAction(cc.Sequence:create(cc.DelayTime:create(1.0), cc.CallFunc:create(function ()
            ...
            --self:getRootNode():getApp():enterSceneEx(appdf.CLIENT_SRC.."plaza.views.LogonScene","FADE",1)
			AppFacade:getInstance():sendNotification(GAME_COMMAMD.POP_VIEW, {Name = VIEW_LIST.BINGDING_REGISTER})
