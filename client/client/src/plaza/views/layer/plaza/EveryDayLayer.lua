...
--获取父场景节点(ClientScene)
function EveryDayLayer:getRootNode()
...
function EveryDayLayer:onButtonClickedEvent(tag,sender)
	...
	if tag == EveryDayLayer.BT_CELL then
		...
		if idx == EveryDayLayer.BT_CHECKIN then
			self:getRootNode():onChangeShowMode(yl.SCENE_CHECKIN)
		elseif idx == EveryDayLayer.BT_ROLLTABLE then
			self:getRootNode():onChangeShowMode(yl.SCENE_TABLE)
		...
		elseif idx == EveryDayLayer.BT_TASK then
			self:getRootNode():onChangeShowMode(yl.SCENE_TASK)
		end
	end
end
...
function EveryDayLayer.tableCellAtIndex(view, idx)	
	...
	if not cell then
    	...
    	--提示
		local clip = ClipText:createClipText(cc.size(720, 30), item.str, "base/fonts/round_body.ttf", 28)
		clip:setTag(3)
