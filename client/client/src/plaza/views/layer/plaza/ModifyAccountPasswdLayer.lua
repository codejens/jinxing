...
local ModifyAccountPasswdLayer = class("ModifyAccountPasswdLayer", cc.BaseLayer)
local ExternalFun = appdf.req(appdf.EXTERNAL_SRC .. "ExternalFun")

function ModifyAccountPasswdLayer:ctor()
	self.super.ctor(self)
	
    -- 加载csb资源
	local csbPath = "modify/ModifyAccountPassLayer.csb"
	...
	Layoutlistener:registerScriptHandler(function(touch, event)
		...
		if false == cc.rectContainsPoint(rec, pos) then
			AppFacade:getInstance():sendNotification(GAME_COMMAMD.POP_VIEW, {Name = VIEW_LIST.MODIFY_ACCOUNT_PASS_LAYER})
		end
		return true
...
function ModifyAccountPasswdLayer:onWillViewEnter()
	...
	if GlobalUserItem.dwUserID	~= 0 then
...
function ModifyAccountPasswdLayer:onWillViewExit()
...
function ModifyAccountPasswdLayer:onButtonClickedEvent(sender)
...
function ModifyAccountPasswdLayer:onModifyCallBack( result, tips )
...
function ModifyAccountPasswdLayer:clearEdit(  )
	self.m_editcode:setText("")
	self.m_editNewPass:setText("")
	self.m_editConfirmPass:setText("")
end

function ModifyAccountPasswdLayer:confirmPasswd(  )
...
--获取手机验证码
function ModifyAccountPasswdLayer:getVerificationCode()
...
function ModifyAccountPasswdLayer:sendloginPassInfo()
...
function ModifyAccountPasswdLayer:sendbankPassInfo()
	local ModifyPassData = ExternalFun.create_netdata(logincmd.CMD_MB_ModifyPassVerifyCode)
	ModifyPassData:setcmdinfo(yl.MDM_MB_LOGON,yl.SUB_MB_MODIFY_PASS_VERIFYCODE)
...
function ModifyAccountPasswdLayer:showToast(msg)
	local curScene = cc.Director:getInstance():getRunningScene()
	showToast(curScene, msg, 1)
end

return ModifyAccountPasswdLayer