...
function OrderRecordLayer:onEnterTransitionFinish()
	...
	local this = self
	appdf.onHttpJsionTable(BaseConfig.WEB_HTTP_URL .. "/WS/MobileInterface.ashx","GET","action=getorderrecord&userid="..GlobalUserItem.dwUserID.."&signature="..GlobalUserItem:getSignature(os.time()).."&time="..os.time().."&number=20&page=1",function(jstable,jsdata)
...
function OrderRecordLayer:ctor(scene)
	...
	--无记录提示
	self._nullTipLabel = cc.Label:createWithTTF("没有兑换记录","base/fonts/round_body.ttf",32)
			:move(yl.WIDTH/2,326)
...
function OrderRecordLayer.tableCellAtIndex(view, idx)	
	...
	if not cell then
		...
		cc.Label:createWithTTF(date,"base/fonts/round_body.ttf",28)
			:move(118,height/2)
			:setVerticalAlignment(cc.VERTICAL_TEXT_ALIGNMENT_CENTER)
			:setTextColor(cc.c4b(206,175,255,255))
			:setAnchorPoint(cc.p(0.5,0.5))
		...
		cc.Label:createWithTTF(item.name,"base/fonts/round_body.ttf",28)
		...
		cc.Label:createWithTTF(string.formatNumberThousands(item.count,true,","),"base/fonts/round_body.ttf",28)
		...
		cc.Label:createWithTTF(string.formatNumberThousands(item.price,true,",").."元宝","base/fonts/round_body.ttf",28)
		...
		cc.Label:createWithTTF(item.tradeType,"base/fonts/round_body.ttf",28)
