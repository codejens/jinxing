	...
	elseif (senderName == "btn_recharge") then
		...
		if (amount == 0) then
			...
			if (self._selectType == yl.PAYMENT_APPLE or self._selectType == yl.PAYMENT_APPSTORE_APPLE) then
				...
				--[[if self._selectType == yl.PAYMENT_CZF_ZF  then
				    local function payCallBack(param)
                        if type(param) == "string" and "true" == param then
                           GlobalUserItem.setTodayPay()
                           showToast(self, "支付成功", 2)
						   self:emit("V2M_PaySuccess")
                           GlobalUserItem.dUserBeans = GlobalUserItem.dUserBeans + item.count --更新用户游戏豆
                           --更新用户游戏豆       
                           local eventListener = cc.EventCustom:new(yl.RY_USERINFO_NOTIFY)
                           eventListener.obj = yl.RY_MSG_USERWEALTH
                           cc.Director:getInstance():getEventDispatcher():dispatchEvent(eventListener)
				    ...
				    payparam["price"] = amount
				    payparam["name"] = ""
                    payparam["paytype"] = "ICBC"
                    MultiPlatform:getInstance():thirdPartyPay(yl.ThirdParty.CZF, payparam, payCallBack)
				else]]
				   ...
				   --  cc.Application:getInstance():openURL(url)
				    ...
				    -- if self._selectType <=yl.PAYMENT_YY_WXGZH and self._selectType <=yl.PAYMENT_YY_JDZF then
                      --   local url = string.format("%s/Pay/uepay/send.aspx?userid=%d&money=%s&ip=%s&pricetype=3&banktype=%s", yl.PAY_SERVER,GlobalUserItem.dwUserID, amount,string.urlencode(GlobalUserItem.szIpAdress),yl.PayString[self._selectType].nameStr )
				    -- else
				    -- 	 local url = string.format("%s/Pay/jrpay/send.aspx?userid=%d&money=%s&ip=%s&pricetype=3&banktype=%s", yl.PAY_SERVER,GlobalUserItem.dwUserID, amount,string.urlencode(GlobalUserItem.szIpAdress),yl.PayString[self._selectType].nameStr )
				    -- end
                  --  local url = string.format("%s/Pay/yifu/send.aspx?userid=%d&money=%s&ip=%s&pricetype=3&banktype=%s", yl.PAY_SERVER,GlobalUserItem.dwUserID, amount,string.urlencode(GlobalUserItem.szIpAdress),yl.PayString[self._selectType].nameStr )
				     cc.Application:getInstance():openURL(url)
