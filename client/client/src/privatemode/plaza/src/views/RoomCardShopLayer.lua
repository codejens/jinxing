...
function RoomCardShopLayer:initBuyUI(csbNode)
    ...
    if nil ~= self.m_tabParam.item.bean and 0 ~= self.m_tabParam.item.bean then
        cc.Label:createWithTTF(string.formatNumberThousands(self.m_tabParam.item.bean,true,",").."游戏豆", "base/fonts/round_body.ttf", 24)
    ...
    if nil ~= self.m_tabParam.item.gold and 0 ~= self.m_tabParam.item.gold then
        ...
        cc.Label:createWithTTF(string.formatNumberThousands(self.m_tabParam.item.gold,true,",").."游戏币", "base/fonts/round_body.ttf", 24)
            :setAnchorPoint(cc.p(0.0,0.5))
            :move(144,y)
            :setTextColor(cc.c4b(255,255,0,255))
            :addTo(buyUi)
    ...
    -- 折扣
    self._txtDiscount = cc.Label:createWithTTF(self.m_discount .. "%折扣", "base/fonts/round_body.ttf", 24)
