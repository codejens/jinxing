...
function RoomDetailLayer:ctor( tabDetail )
    ...
    -- 房间战绩列表
    if onGame then
        local clipGrad = ClipText:createClipText(cc.size(373, 28), "该房间正在游戏中,暂无战绩!", "base/fonts/round_body.ttf", 24)
        ...
        for i = 1, count do
            ...
            if 0 == v.dwUserID then
                if 1 == i then
                    local clipGrad = ClipText:createClipText(cc.size(373, 28), "该房间无战绩, 房间已解散", "base/fonts/round_body.ttf", 24)
                    clipGrad:setAnchorPoint(cc.p(0, 0.5))
            ...
            local clipGrad = ClipText:createClipText(cc.size(150, 28), str, "base/fonts/round_body.ttf", 24)
            clipGrad:setAnchorPoint(cc.p(0, 0.5))
            local pos = posList[i] or cc.p(0,0)
            clipGrad:setPosition(pos)
            image_bg:addChild(clipGrad)
            ...
            clipGrad = ClipText:createClipText(cc.size(100, 28), str, "base/fonts/round_body.ttf", 24)
            clipGrad:setAnchorPoint(cc.p(0, 0.5))
            local pos = posList[i] or cc.p(0,0)
        ...
        PriRoom:getInstance():getPlazaScene():popTargetShare(function(target, bMyFriend)
            ...
            if bMyFriend then
                PriRoom:getInstance():getTagLayer(PriRoom.LAYTAG.LAYER_FRIENDLIST, function( frienddata )
                    ...
                    PriRoom:getInstance():priInviteFriend(frienddata, GlobalUserItem.getCurGameKind(), serverid, yl.INVALID_TABLE, msgTab.friendContent)
                end)
            elseif nil ~= target then
