...
local RoomIdInputLayer = class("RoomIdInputLayer", function(scene)
		local RoomIdInputLayer = display.newLayer(cc.c4b(0, 0, 0, 125))
...
function RoomIdInputLayer:ctor()
	...
	--关闭按钮
	self.btnreturn = bg:getChildByName("Button_Return")
	self.btnreturn:setTag(BTN_RETURN)
	self.btnreturn:addTouchEventListener(callback)
