...
--win32是否需要更新 标识
local WIN32_NEED_CHECK_UPDATE = true
...
function WelcomeScene:checkWebUrlInfo(func)
	...
	print("webUrlInfowebUrlInfowebUrlInfowebUrlInfowebUrlInfowebUrlInfo=",webUrlInfo)
    ...
    if not result then
		...
		local function updateCallBack(needupdate)
			if (not needupdate) then
				printf("11无需更新，进入登录，当前资源版本号：%d", self._newResVersion)
