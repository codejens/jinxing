...
function ShopMediator:onQueryAgentInfo_User()
    ...
    appdf.onHttpJsionTable(url ,"GET","action=getchild&gameid=" .. GlobalUserItem.dwGameID,function(jstable,jsdata)
		...
		--if type(jstable) == "table" then
			...
			if tonumber(code) == 1 then
				...
				--	local valid = datax["valid"]
					--if valid == true then
						local list = jstable["msg"]--msg["gameid"]
						--if type(list) == "table" then
							local agentRechargeList = {}
							for i=1,#list do
								local item = {}
								item.gameid = msg[i]["gameid"]
                                showToast(self,item.gameid,2,cc.c3b(250,0,0))
								--item.name = list[i]["ShowName"]
							--	item.wechat = list[i]["WeChat"]
								table.insert(agentRechargeList, item)
							end
							self.viewComponent:emit("M2V_QueryAgentResult_User", agentRechargeList)
					--	end
					--end
				--end
			end
