...
--[[	btn = ExternalFun.getChildRecursiveByName(self.m_moreNode, "tru_btn")
	...
	--设置按钮
	btn = ExternalFun.getChildRecursiveByName(self.m_moreNode, "set_btn")
	btn:setTag(TAG_ENUM.BT_SET)
	btn:setSwallowTouches(true)
	btn:addTouchEventListener(btnEvent)
	
	--退出按钮
	btn = ExternalFun.getChildRecursiveByName(self.m_moreNode, "back_btn")
	self.m_btn_back = btn
...
--[[	if GameLogic.CT_THREE_LINE == cardType then             -- 飞机
	...
	if GameLogic.CT_THREE_LINE == cardType then             -- 飞机
		local plane_node = self.m_special_card_node:getChildByName("plane")
		local sprite = plane_node:getChildByName("sprite")
		local head =  PopupInfoHead:createNormal(self:getUserItem(outViewId), 43)
		sprite:addChild(head)
		head:setPosition(sprite:getChildByName("head"):getPosition())
		self.m_special_card_ani:play("plane", false)
	...
	elseif GameLogic.CT_MISSILE_CARD == cardType then       -- 火箭
		local plane_node = self.m_special_card_node:getChildByName("rocket")
		local sprite = plane_node:getChildByName("sprite")
		local head =  PopupInfoHead:createNormalCircle(self:getUserItem(outViewId), 75,("Circleframe.png"))
		sprite:addChild(head)
		head:setPosition(sprite:getChildByName("head"):getPosition())
		self.m_special_card_ani:play("rocket", false)
...
--[[	-- 显示准备
	...
	elseif  nil ~= rs.m_fanchuntian_multiple then
		self.m_spring_ani:setLastFrameCallFunc(callback)
		self.m_spring_ani:play("fanchun", false)
		ExternalFun.playSoundEffect( "spring.wav" )
	else
		callback()
	end
