...
--玩家列表按钮位置
local userlistpoint = cc.p(161, 144)
...
function GameViewLayer:loadResource()
    ...
    --菜单按钮
    local  btn = self.m_scbNode:getChildByName("bt_pull")
   ...
   --[[ --座位
	...
	self.m_cardLayer = csbNode:getChildByName("cardLayer")
    local gupai = 0
    ...
    --游戏币层--下注的筹码层
	self.m_goldLayer = csbNode:getChildByName("goldLayer")
...
function GameViewLayer:GameMainDraw()
	...
	--色子
	self.m_sicebg1 = self.m_scbNode:getChildByName("sicebg1")
	self.m_sicebg1:setVisible(false)
		
	self.m_sicebg2 = self.m_scbNode:getChildByName("sicebg2")
	...
	--无座玩家列表
	self.bt_nositplayer = self.m_scbNode:getChildByName("bt_nositplayer")
    self.bt_nositplayer:setTag(TAG_ENUM.BT_USERLIST)
    self.bt_nositplayer:addTouchEventListener(btnEvent)
	
	--3轮总下注及自己下注
	self.m_bets_tj_bg = self.m_scbNode:getChildByName("im_bets_tj_bg")
	...
	--开牌记录
	self.bt_cardrecord = self.m_scbNode:getChildByName("bt_cardrecord")
	...
	for i=1,4 do
        ...
        local SituserResult = self.m_scbNode:getChildByName(str)
...
function GameViewLayer:OnUpdataClockView(chair, time)
    ...
    if chair == self:getParentNode():SwitchViewChairID(selfchair) then
        ...
        elseif time == self.m_cbTimeLeave-2 then
            --发牌处理
            self:sendCard(true)
		elseif time == self.m_cbTimeLeave-3 then
			self.m_sicebg1:stopAllActions()
			self.m_sicebg2:stopAllActions()
			self.m_sicebg1:setVisible(false)
			self.m_sicebg2:setVisible(false)
...
function GameViewLayer:showMenu()
	--MXM修复官方原本BUG
	local btpull = self.m_scbNode:getChildByName("bt_pull")
	if self.m_bshowMenu == false then
        self.m_bshowMenu = true
