...
--        self.btChip[i]:setScale(0.6)
	...
	--底部显示的文字
	self.m_operatorText =  cc.Label:createWithTTF("", "base/fonts/round_body.ttf", 32)
...
function GameViewLayer:ShowUserChat(viewid ,message) 
	if message and #message > 0 then 
		...
		local labCountLength = cc.Label:createWithTTF(message,"base/fonts/round_body.ttf", 20)   
		if labCountLength:getContentSize().width > limWidth then 
			self.m_UserChat[viewid] = cc.Label:createWithTTF(message,"base/fonts/round_body.ttf", 20, cc.size(limWidth, 0)) 
		else 
			self.m_UserChat[viewid] = cc.Label:createWithTTF(message,"base/fonts/round_body.ttf", 20) 
		end 
